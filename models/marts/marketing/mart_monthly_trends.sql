...
final as (
    ...
    select
        ...
        {{ safe_divide(
            "gross_revenue - lag(gross_revenue) over (partition by category order by order_year, order_month)",
            "lag(gross_revenue) over (partition by category order by order_year, order_month)"
